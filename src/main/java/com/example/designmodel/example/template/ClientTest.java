package com.example.designmodel.example.template;
 ...
 *
 * 模板模式 通过在父类中定义一个方法的固定算法结构（模板方法），并将一些步骤的实现交给子类来完成，使得子类可以在不改变算法结构的前提下，定制具体的行为。
 * 它适用于 算法结构固定，但某些细节可以由 子类自定义 的场景。
